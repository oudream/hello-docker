<template>
	<section>
		...
		<el-table ref="mainTable" :data="objs" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="version" label="年版" width="100" sortable>
			</el-table-column>
		...
		<el-col :span="24" class="toolbar">
			...
			<el-radio-group v-model="mode_sel">
				<el-radio class="radio" :label="0">单选</el-radio>
				<el-radio class="radio" :label="1">复选</el-radio>
			</el-radio-group>
		...
		<!-- edit -->
		<el-dialog title="地图档案编辑" :visible.sync="editFormVisible" :close-on-click-modal="false" v-dialog-drag top="1vh">
		...
		<!-- add -->
		<el-dialog title="地图档案新增" :visible.sync="addFormVisible" :close-on-click-modal="false" v-dialog-drag top="1vh">
			<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				<el-form-item label="图名" prop="name">
					<el-input v-model="addForm.name" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="编号" prop="uri">
				...
				<el-form-item label="年版" prop="version">
					<!--<el-date-picker type="date" placeholder="选择日期" v-model="editForm.version"></el-date-picker>-->
					<el-input v-model="addForm.version"></el-input>
				</el-form-item>
				<!--<el-form-item label="位置">-->
					<!--<el-input v-model="addForm.pos"></el-input>-->
				<!--</el-form-item>-->
	...
	export default {
		...
		data() {
			return {
				...
				addFormRules: {
					name: [
						{ required: true, message: '请输入图名', trigger: 'blur' }
					],
                    uri: [
                    ...
                    version: [
                        { required: true, message: '请输入年版号', trigger: 'blur' }
                    ]
				},
				//新增界面数据
				addForm: {
                ...
                cfg_ti_st:{
					...
					cnStd:{name:'f_cnt_std',desc:'数量',type:'number',def:0},
                    cnGood:{name:'f_cnt_good',desc:'完好数量',type:'number',def:0},
                    cnBad:{name:'f_cnt_bad',desc:'损坏数量',type:'number',def:0},
                    cnLost:{name:'f_cnt_lost',desc:'丢失数量',type:'number',def:0},
                    cnLend:{name:'f_cnt_lend',desc:'借出数量',type:'number',def:0},
//                    userName:{name:'',desc:'用户'},
//					userId:{name:'f_id_user',desc:'用户id'},
//					userDT:{name:'f_t_user',desc:'时间',type:'now_ms'},
                    res0:{name:'f_res0',desc:'保留0',type:'number',def:0},
                    res1:{name:'f_res1',desc:'保留1'},
                    user_crt:{name:'f_user_crt',def:'sys'},
                    t_crt:{name:'f_t_crt',type:'now_ms'},
                ...
                cfg_const:{
                    ...
                    class:'DA',
                    user:'SYS',
                    sql_sel:'SELECT id as `id`,f_id as `uuid`,f_count as `count`, f_name as `name`,f_uri as `uri`,f_grade as `grade`,f_ver as `version`,f_desc as `remark`, f_scale as `scale` FROM ti_obj_da ',
                    sql_max_id:'SELECT MAX(id) as max_id FROM ti_obj_da',
                    sql_del:'update ti_obj_da set f_dt_flag = -1 WHERE id = ',
                    sql_rec_cnt:' SELECT COUNT(id) as count_rec FROM ti_obj_da ',
                    sql_where_0:' WHERE {0}  LIKE \'%{0}%\' and f_dt_flag>=0 ',
                    sql_where_1:" WHERE f_dt_flag>=0 ",
                    sql_del_st:' update ti_rt_da set f_dt_flag = -1 WHERE f_id = \'{0}\' ',
...
//					}
			...
			handleDel: function () {
				...
				this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    //状态表
                    sql_0 =   util.formatString.format(this.cfg_const.sql_del_st,obj.uuid);
                    para.sqls.push(sql_0);
                    debugger
                    let v_log = {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            r = dbCommon.sql.getInsert(this.cfg_const.st,this.cfg_ti_st,this.addForm);
                            para.sqls.push(r.sql);
			...
			batchRemove: function () {
				...
				this.$confirm('确认删除选中记录吗？', '提示', {
                    ...
                    this.sels.map(ele=>{
                        ...
                        sql = this.cfg_const.sql_del_st + ele.uuid;
                        para.sqls.push(sql);
                        //写日志
                        let v_log = {
                            uuid:util.uuid.create(0),
