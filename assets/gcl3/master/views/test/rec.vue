	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" >
				...
				<el-form-item>
					<el-button type="primary" @click="handleAddBatch">批量新增</el-button>
				</el-form-item>
			</el-form>
		</el-col>
		...
		<el-table :data="objs" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="grade" label="等级" width="120" :formatter="formatGrade" sortable>
			</el-table-column>
			<el-table-column prop="scale" label="比例" width="120" sortable>
			</el-table-column>
			<el-table-column prop="version" label="版号" width="120" sortable>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
                ...
                cfg_const:{
                    ...
                    class:'DA',
                    user:'SYS',
                    sql_sel:'SELECT id as `id`,f_id as `uuid`,f_name as `name`,f_uri as `uri`,f_grade as `grade`,f_ver as `version`,f_desc as `remark`, f_scale as `scale` FROM ti_obj_da ',
                    ...
                    sql_del_st:' update ti_da_st set f_dt_flag = -1 WHERE f_id = \'{0}\' ',
		...
		methods: {
			...
			formatGrade: function (row, column) {
			    if (row[column.property] ==1) return '普密';
...
//					}
            ...
            handleAddBatch: function (){
                this.$confirm('确认批量新增吗？', '提示', {}).then(() => {
                    ...
                    sql = 'delete from '+this.cfg_const.st +';';
                    para.sqls.push(sql);
                    ...
                    for(let i=1;i<=5;i++){
                        for (let j=1;j<=7;j++){
                            for (let k=1;k<=3;k++){
                                let e = {
                                    uuid:util.uuid.create(0),
                                    name: Mock.mock('@county(false)'),
                                    uri: Mock.Random.date(),
                                    grade:Mock.Random.integer(1, 3),
                                    scale:'1:100',
                                    version:Mock.Random.date(),
                                    press:'',
                                    posId: [i,j,k].join('-'),
                                    remark: '',
                                ...
                                para.sqls.push(r.sql);
                                r = dbCommon.sql.getInsert(this.cfg_const.st,this.cfg_ti_st,e);
                                para.sqls.push(r.sql);
                                debugger
                            }
                        }
