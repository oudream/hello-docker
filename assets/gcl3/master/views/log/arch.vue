	...
	export default {
		...
		data() {
			return {
                ...
                cfg_const:{
					...
					'a.f_obj as `obj`,a.f_v as `v`,b.f_name as `name_3rd` ' +
					'from ti_log_da a left join ti_obj_da b on a.f_pid = b.f_id',
                    sql_max_id:'SELECT MAX(a.id) as max_id FROM ti_log_da a',
                    sql_del:'update ti_log_da a set a.f_dt_flag = -1 WHERE a.id = ',
                    sql_rec_cnt:'SELECT COUNT(a.id) as count_rec FROM ti_log_da a',
                    sql_where_0:' WHERE a.{0}  LIKE \'%{1}%\' and a.f_dt_flag=0 and a.f_t>={2} and a.f_t<{3}',
                    sql_where_1:" WHERE a.f_dt_flag=0 and a.f_t>={0} and a.f_t<{1}",
		...
		methods: {
            ...
            formatCode: function (row, column) {
                if (row[column.property] ==0) return '增加';
                else if(row[column.property] ==1) return '修改';
                else if(row[column.property] ==2) return '删除';
                else if(row[column.property] ==3) return '批删';
                else return '未知';
            },
			// double
