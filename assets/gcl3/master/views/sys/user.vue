<template>
	<section>
		...
		<!-- edit -->
		<el-dialog title="人员档案编辑" :visible.sync="editFormVisible" :close-on-click-modal="false" v-dialog-drag top="1vh">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="级别">
					<el-radio-group v-model="editForm.grade">
						<el-radio class="radio" :label="1">干部</el-radio>
						<el-radio class="radio" :label="2">士官</el-radio>
						<el-radio class="radio" :label="3">义务兵</el-radio>
					</el-radio-group>
				</el-form-item>
		...
		<!-- add -->
		<el-dialog title="人员档案新增" :visible.sync="addFormVisible" :close-on-click-modal="false" v-dialog-drag top="1vh">
...
//                    st_flag:{name:'f_st_flag',type:'number',def:0},
                ...
                cfg_const:{
                    ...
                    class:'USER',
                    user:'SYS',
                    sql_sel:'SELECT id as `id`,f_id as `uuid`, f_name as `name`,f_uri as `uri`,f_grade as `grade`,f_sex as `sex`,f_age as `age`,f_tel as `tel`,f_desc as `remark`, f_st_flag as `status` FROM ti_obj_user ',
                    sql_max_id:'SELECT MAX(id) as max_id FROM ti_obj_user',
                    sql_del:'update ti_obj_user set f_dt_flag = -1 WHERE id = ',
                    sql_rec_cnt:'SELECT COUNT(id) as count_rec FROM ti_obj_user ',
		...
		methods: {
			...
			formatGrade: function (row, column) {
			    if (row[column.property] ==1) return '干部';
			    else if(row[column.property] ==2) return '士官';
                else if(row[column.property] ==3) return '义务兵';
                else return '未知';
			},
			//sex
