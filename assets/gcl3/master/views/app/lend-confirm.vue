<template>
	<section>
		...
		<el-table ref="mainTable" :data="objs" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="count" label="总数" width="100" sortable>
			</el-table-column>
			<el-table-column prop="cntStd" label="借用数量" width="100" sortable>
			</el-table-column>
			<el-table-column prop="readerName" label="借用人" width="100" sortable>
			</el-table-column>
			<el-table-column prop="monName" label="监管人" width="100" sortable>
			</el-table-column>
			...
			<el-form :model="detailForm" label-width="80px" :rules="detailFormRules" ref="detailForm">
				...
				<el-form-item label="保密级别">
					<el-radio-group v-model="detailForm.grade" :disabled = "true" >
						<el-radio class="radio" :label="1">普通</el-radio>
		...
		data() {
			return {
                ...
                cfg_const:{
                    ...
                    sql_where_0:'WHERE a.{0}  LIKE \'%{0}%\' and a.f_dt_flag=0 and d.f_code=1 and d.f_syn_flag = 1 ',
                    sql_where_1:" WHERE a.f_dt_flag=0 and d.f_code=1 and d.f_syn_flag = 1 ",
				...
				this.$refs.actForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认借用吗？', '提示', {}).then(() => {
                            ...
                            let sv = util.formatString.format(' set f_status = {0},f_syn_flag = {1},f_cnt_std = f_cnt_std - {2},f_cnt_lend = f_cnt_lend + {3} ',1,1,obj.cntStd,obj.cntStd);
                            let cond = util.formatString.format(' where f_id = \'{0}\'', obj.uuid );
                            r2.sql = 'update '+ this.cfg_const.st + sv + cond;

//                            let conds2 = 'f_id = \''+ obj.uuid+'\'';
//                            this.actForm.status = 1;
