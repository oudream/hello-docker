<template>
    <section>
        ...
        <!-- table -->
        <el-table ref="mainTable" :data="objs" highlight-current-row v-loading="listLoading"
        ...
        <!-- act -->
        <el-dialog title="地图借用" :visible.sync="actFormVisible" :close-on-click-modal="false" v-dialog-drag top="1vh">
            ...
            <el-form :model="actForm" label-width="60px" :rules="actFormRules" ref="actForm">
                ...
                <!--<validator name="validation_user">-->
                    <el-form-item label="用户" prop="readerName">
                        <el-input v-model="actForm.readerName"  :disabled="true" ref="readerName"  >
                            <el-button slot="append" icon="el-icon-search"
                                       @click.native="actReaderSelVisible=true"></el-button>
                        </el-input>
                        <readerSel :visible.sync="actReaderSelVisible" :sels_ids="[actForm.readerId]"
                                   @submit="handleReaderFormSubmit"></readerSel>
                    </el-form-item>
                    <el-form-item label="监管"  prop="monName">
                        <el-input v-model="actForm.monName"  :disabled="true" ref="monName">
                            <el-button slot="append" icon="el-icon-search"
                                       @click.native="actMonSelVisible=true"></el-button>
                        </el-input>
                        <monSel :visible.sync="actMonSelVisible" :sels_ids="[actForm.monId]"
                                @submit="handleMonFormSubmit"></monSel>
                    </el-form-item>
                <!--</validator>-->
                <el-form-item label="数量" prop="cntLend">
                    <el-input-number v-model="actForm.cntLend"></el-input-number>
                </el-form-item>
            </el-form>

            <div slot="footer" class="dialog-footer">
                <el-button @click.native="actFormVisible = false">取消</el-button>
                <el-button type="primary" @click.native="actSubmit"  :loading="actLoading">借用</el-button>
            </div>
        </el-dialog>

        <!-- detail -->
        <el-dialog title="地图档案详情" :visible.sync="detailFormVisible" :close-on-click-modal="false" v-dialog-drag
                   top="1vh">
...
<script>
    ...
    import Toast from './../../../../3rd/cvue-3/component/mi_toast.vue'
    ...
    export default {
        ...
        data() {
            return {
                ...
                actFormRules: {
//                    readerName: [
//                        { required: true, message: '借用用户必选', trigger: ['change','blur'] }
//                    ],
//                    monName: [
//                        { required: true, message: '监管用户必选', trigger: ['change','blur'] }
//                    ],
                    cntLend: [
                        { required: true, message: '数量必须大于0', trigger: 'blur'},
                        {validator:(rule, value, callback)=> {
                           if (value<=0) callback('数量必须大于0!!!');
                           else callback();
                        }, trigger: 'blur'}
                    ],
                },
                ...
                cfg_const: {
                    ...
                    sql_rec_cnt: 'SELECT COUNT(a.id) as count_rec FROM ti_obj_da a left join ti_rt_da b on a.f_id=b.f_id ',
                    sql_where_0: 'WHERE a.{0}  LIKE \'%{0}%\' and a.f_dt_flag=0 and (b.f_cnt_std-b.f_cnt_lend) > 0 ',
                    sql_where_1: " WHERE a.f_dt_flag=0 and (b.f_cnt_std-b.f_cnt_lend) > 0 ",
...
//					}
            ...
            handleAction: function () {
                ...
                this.actForm.monName='';
                this.actForm.readerName='';
            },
...
//                    this.$refs.monName.value = a[0].name;
                    debugger
                }
            },
...
//                    this.$refs.readerName.onChange();
            ...
            actSubmit: function () {
                ...
                if (obj.readerName === undefined || obj.readerName ===''){
                    this.$confirm('借用人不能为空', '警告', {});
                    return;
                }
                debugger
                if(obj.monName === undefined || obj.monName === ''){
                    this.$confirm('监管人不能为空', '警告', {});
                    return;
                }
                debugger
