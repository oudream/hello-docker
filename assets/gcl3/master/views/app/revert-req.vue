	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" >
				...
				<el-form-item>
					<el-button type="primary"  @click="handleAction" :disabled="this.sels.length===0" >申请</el-button>
				</el-form-item>
				<!--<el-form-item>-->
		...
		<el-table ref="mainTable" :data="objs" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="grade" label="等级" width="100" :formatter="formatGrade" sortable>
			</el-table-column>
			<el-table-column prop="pos" label="位置" width="100" sortable>
			</el-table-column>
			<el-table-column prop="count" label="总数" width="80" sortable>
			</el-table-column>
			<el-table-column prop="cntStd" label="借用数量" width="100" sortable>
			</el-table-column>
			<el-table-column prop="readerName" label="借用人" width="120" sortable>
			</el-table-column>
			<el-table-column prop="monName" label="监管人" width="120" sortable>
			</el-table-column>
			<el-table-column prop="readerDT" label="借用时间" width="180" sortable>
			</el-table-column>
		...
		<!-- act -->
		<el-dialog title="地图归还" :visible.sync="actFormVisible" :close-on-click-modal="false"  v-dialog-drag top="1vh">
			<el-form :model="actForm" label-width="60px" :rules="actFormRules" ref="actForm">
				<el-form-item label="图名" prop="name">
					<el-input v-model="actForm.name" :disabled = "true"  auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="位置">
					<el-input v-model="actForm.pos" :disabled = "true" ></el-input>
				</el-form-item>
				<!--<el-form-item label="状态">-->
					<!--<el-radio-group v-model="actForm.status" :disabled = "true" >-->
				...
				<el-form-item label="总数">
					<el-input v-model="actForm.count" :disabled = "true" ></el-input>
				</el-form-item>
				<el-form-item label="借用人">
					<el-input v-model="actForm.readerName" :disabled = "true">
						<el-button slot="append" icon="el-icon-search" @click.native="actReaderSelVisible=true"></el-button>
					</el-input>
					<readerSel :visible.sync="actReaderSelVisible" :sels_ids="[actForm.readerId]" @submit="handleReaderFormSubmit"></readerSel>
				</el-form-item>
				<el-form-item label="监管人">
					<el-input v-model="actForm.monName" :disabled = "true">
						<el-button slot="append" icon="el-icon-search" @click.native="actMonSelVisible=true"></el-button>
					</el-input>
					<monSel :visible.sync="actMonSelVisible" :sels_ids="[actForm.monId]" @submit="handleMonFormSubmit"></monSel>
				</el-form-item>
				<el-form-item label="数量">
					<el-col :span="3" align="center">归还</el-col>
					<el-col :span="3" >
						<el-input v-model="actForm.cntStd" ></el-input>
					</el-col>
				</el-form-item>
                <el-form-item label="质量">
					<el-col :span="3" align="center">完好</el-col>
					<el-col :span="3" >
						<el-input v-model="actForm.cntGood" ></el-input>
					</el-col>

					<el-col :span="3" align="center">破损</el-col>
					<el-col :span="3">
						<el-input v-model="actForm.cntBad" ></el-input>
					</el-col>

					<el-col :span="3" align="center">丢失</el-col>
					<el-col :span="3">
						<el-input v-model="actForm.cntLost" ></el-input>
					</el-col>
				</el-form-item>
				<el-form-item label="备注">
				<el-input type="textarea" v-model="actForm.remark" ></el-input>
				</el-form-item>
			</el-form>

			<div slot="footer" class="dialog-footer">
				<el-button @click.native="actFormVisible = false">取消</el-button>
				<el-button type="primary" @click.native="actSubmit" :loading="actLoading">归还</el-button>
			</div>
		</el-dialog>

		<!-- detail -->
		<el-dialog title="地图档案详情"  :visible.sync="detailFormVisible" :close-on-click-modal="false" v-dialog-drag top="1vh">
...
<script>
	...
	import readerSel from '../sys/reader-sel.vue';
    import monSel from '../sys/user-sel.vue';
    ...
    export default {
		...
		data() {
			return {
                ...
                actMonSelVisible: false,
                actReaderSelVisible: false,
                ...
                cfg_ti_his:{
                    ...
                    readerName:{name:'',desc:'借用用户'},
                    readerId:{name:'f_id_reader',desc:'借用用户id'},
                    readerDT:{name:'f_t_reader',desc:'借用时间',type:'now_ms'},
                    remark:{name:'f_desc',desc:'备注'},
                ...
                cfg_const:{
                    ...
                    'b.f_status as `status`, c.f_name as `pos`,c.f_id as `posId`,' +
                    'd.f_cnt_std as `cntStd`,d.f_cnt_lost as `cntLost`,d.f_cnt_good as `cntGood`,d.f_cnt_bad as `cntBad`,d.f_id as `hisId`,from_unixtime(d.f_t_reader/1000,\'%Y-%m-%d %H:%i:%s\') as `readerDT`,' +
                    'd.f_id_mon as `monId` ,d.f_id_reader as `readerId`,e.f_name as  `monName`,f.f_name as `readerName` ' +
                    'FROM ti_obj_da a left join ti_rt_da b on a.f_id=b.f_id '+
                    'left join ti_obj_cab c on a.f_id_pos=c.f_id '+
                    'left join ti_his_da  d on a.f_id = d.f_pid ' +
                    'left join ti_obj_user  e on d.f_id_mon = e.f_id ' +
                    'left join ti_obj_reader  f on d.f_id_reader  = f.f_id ',
                    sql_max_id:'SELECT MAX(a.id) as max_id FROM ti_obj_da a',
                    sql_del:'update ti_obj_da a set a.f_dt_flag = -1 WHERE id = ',
                    sql_rec_cnt:'SELECT COUNT(a.id) as count_rec FROM ti_obj_da a left join ti_rt_da b on a.f_id=b.f_id left join ti_his_da  d on a.f_id = d.f_pid ',
                    sql_where_0:'WHERE a.{0}  LIKE \'%{0}%\' and a.f_dt_flag=0 and d.f_code=2 and d.f_syn_flag = 1 ',
                    sql_where_1:" WHERE a.f_dt_flag=0 and d.f_code=2 and d.f_syn_flag = 1 ",
                    sql_order:" order by c.f_name ",
                    sql_upd_syn:'update ti_his_da set f_syn_flag = 0 where f_id=\'{0}\';',
...
//					}
            ...
            handleMonFormSubmit: function(a) {
                if (Array.isArray(a)) {
                    this.actForm.monId   = a[0].uuid;
                    this.actForm.monName = a[0].name;
                }
            },
            handleReaderFormSubmit: function(a) {
                if (Array.isArray(a)) {
                    this.actForm.readerId   = a[0].uuid;
                    this.actForm.readerName = a[0].name;
                }
            },
			// submit act
			actSubmit: function () {
				this.$refs.actForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认归还吗？', '提示', {}).then(() => {
							//
							this.actLoading = true;
							let obj = Object.assign({}, this.actForm);

                            let para = {
                                session: Date.now(),
                                sqls:[]
                            };
                            let sqlUpd = util.formatString.format(this.cfg_const.sql_upd_syn,obj.hisId);

                            let hisData = Object.assign({}, this.actForm);
                            hisData.uuid = util.uuid.create(0);
                            hisData.pid  = obj.uuid;
//                            hisData.cntStd = hisData.cntGood = obj.cntLend;
//                            hisData.cntBad = hisData.cntLost = 0;
                            hisData.code   = 3;
                            hisData.syn_flag = 1;
                            let r1 = dbCommon.sql.getInsert(this.cfg_const.his,this.cfg_ti_his,hisData);
                            ...
                            let sv = util.formatString.format(' set f_status = {0},f_syn_flag = {1} ',11,1);
                            let cond = util.formatString.format(' where f_id = \'{0}\'', obj.uuid );
                            r2.sql = 'update '+ this.cfg_const.st + sv + cond;

//                            let conds2 = 'f_id = \''+ obj.uuid+'\'';
//                            this.actForm.status = 11;
//                            this.actForm.syn_flag = 1;
//                            this.actForm.cntLost = this.actFormOld.cntLost;
//                            let r2 = dbCommon.sql.getUpdate(this.cfg_const.st,this.cfg_ti_st,this.actForm,this.actFormOld,conds2);
                            ...
                            if(ok == true){
                                //更新标志
                                para.sqls.push(sqlUpd);
                                //写日志
                                let v_log = {
                                    uuid:util.uuid.create(0),
                                    pid:this.actForm.uuid,
                                    uri:'upd',
