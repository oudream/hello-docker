    ...
    <section>
        ...
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
            <el-form :inline="true">
                ...
                <el-form-item v-for="(edition , index) in editions" :key="index">
                    <el-button type="primary" @click="handleEdit(edition.name)">
                        {{edition.title}}
                    </el-button>
                </el-form-item>
            </el-form>
            ...
            <el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
                ...
                <el-form-item v-for="(attr, index) in attrs" :key="index" :label="attr.label" :prop="attr.name">
                    <section v-if="attr.from">
                        <el-input v-model="editForm[attr.name]" :ref="'edition_' + attr.name" disabled>
                            <el-button slot="append" icon="el-icon-search" :disabled="attr.readonly"
                    ...
                    <el-input v-else-if="attr.type === 'string' && attr.maxLength < 256" v-model="editForm[attr.name]"
                              auto-complete="off" :ref="'edition_' + attr.name" :disabled="attr.readonly"></el-input>
                    <el-input v-else-if="attr.type === 'string'" type="textarea"
                              v-model="editForm[attr.name]" :ref="'edition_' + attr.name" :disabled="attr.readonly"></el-input>
                    <el-input-number
                            ...
                            :step="0.01" :ref="'edition_' + attr.name" :disabled="attr.readonly"></el-input-number>
                    <el-radio-group v-else-if="attr.type === 'bool'" v-model="editForm[attr.name]"
                                    :ref="'edition_' + attr.name" :disabled="attr.readonly">
                    ...
                    <el-date-picker v-else-if="attr.type === 'date'" type="date" placeholder="选择日期"
                                    v-model="editForm[attr.name]"
                                    :ref="'edition_' + attr.name" :disabled="attr.readonly"></el-date-picker>
                </el-form-item>
            ...
            handleEdit: function(action) {
                ...
                getOdoQuery(params).then((rs) => {
                    if (rs && rs.state.err) {
                        ...
                        if (this.nObj.spec.edition && this.nObj.spec.edition.preset && this.nObj.spec.edition.editions) {
                            this.nObj.spec.edition.preset(action, obj);
                            let edition = this.nObj.spec.edition.editions.find(e => e.name === action);
                            if (edition) {
                                this.editFormTitle = edition.title;
                                if (Array.isArray(edition.attrs)) {
                                    edition.attrs.forEach(ele=>{
                                        let attr = this.attrs.find(a => a.name === ele.name);
                                        if (attr) {
                                            attr.readonly = ele.readonly;
                                        }
                                    });
                                }
                            }
            ...
            sels(nv, ov) {
                if (this.sels.length >0){
                    if (this.nObj.spec.edition && this.nObj.spec.edition.filter && this.nObj.spec.edition.editions) {
                        let obj = this.sels[0];
                        let actions = this.nObj.spec.edition.filter(obj);
                        this.editions = this.nObj.spec.edition.editions.filter(ele => actions.indexOf(ele.name) > -1);
                    }
                } else {
                    this.editions = [];
                }
